  ...
  >
    ...
    >
      ...
      <el-table-column
        ...
        <template v-slot="scope">
          <span>{{ scope.$index + 1 + (listInfo.query.pageNum - 1) * listInfo.query.pageSize }}</span>
        </template>
      </el-table-column>
      <el-table-column
    ...
    <template v-if="pager">
      ...
      >
        <el-pagination
          background
          :current-page.sync="listInfo.query.pageNum"
...
export default {
  ...
  methods: {
    ...
    handleSizeChange (val) {
      ...
      query.pageNum = 1 // 每页条数切换，重置当前页
      this.getList(this.api)
    },
    handleCurrentChange (val) {
      this.listInfo.query.pageNum = val // 当前页
      this.getList(this.api)
    },
    // 派发按钮点击事件
